...
function ShowQuestion(props) {
    ...
    return ( <
        ...
        br / > {
            props.question.i !== "" ? < Image src = { props.question.i }
            alt = "" / >
            :
                ''
        } <
        ShowAnswer onClick = { props.SetAnswer } > < Arrow state = { props.showanswer }
        />Answer</ShowAnswer >
        <
...
function Box(props) {
    ...
    return ( <
        ...
        /FullDiv> {
        visible ? < ShowQuestion question = { props.question }
        visible = { visible }
        closeOnClick = {
            () => {
                setVisible(false);
            ...
            props.qs.map((row, rowIndex, mainArray) => ( <
                tr > {
                    row.map((element, colIndex) => ( <
                        Box value = { `$${( rowIndex + 1) * 100 * props.factor}` }
                        question = { mainArray.at(colIndex).at(rowIndex) } > < /Box>
                    ))
                } <
                /tr>
            ))
...
function Page(props) {
    ...
    return ( <
        ...
        <
        H1 > Aibou Arena < /H1> <
        MoneyGrid cats = { cats.at(currPage) }
        qs = { qs.at(currPage) }
        ...
        <
        NavButton onClick = {
            () => setPage(Math.max(currPage - 1, 0))
        } > previous < /NavButton> <
        NavButton onClick = {
            () => setPage(Math.min(currPage + 1, highestPage))
        } > forward < /NavButton>
        page: { currPage } <
        /NavButtonWrapper> < /
        PageWrap >
